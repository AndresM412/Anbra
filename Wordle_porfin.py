...
def contar_letra(palabra, letra):
    aux = 0
    current_node = palabra.head
    while current_node != None:
        if current_node.value.__eq__(letra[5:-4]) or current_node.value.__eq__(letra): #__eq__ , es igual a ==, solo que este se utiliza para comparar cadenas de caracteres
            aux += 1
        current_node = current_node.next
    return aux


def contar_letraColor(palabra, letra):
    aux = 0
    current_node = palabra.head
    while current_node != None:
        caracter = current_node.value[5:-4]
        if caracter is "":
            caracter = current_node.value

        if caracter is (letra[5:-4]) or caracter is (letra):
            aux += 1
        current_node = current_node.next
    return aux
...
while(Intentos_gastados <= Tope_intentos and Ganar == False):
    ...
    while(L3_node is not None): 
        aux1 = contar_letra(L1, L3_node.value)
        aux2 = contar_letraColor(L4, L3_node.value)
        if(aux1 > aux2):
            Pinte = en_amarillo(L1_node, L3_node)
            L4.add_tail(Pinte)          
        else:
            L4.add_tail(L3_node.value)
